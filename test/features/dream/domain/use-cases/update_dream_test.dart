...
import 'update_dream_test.mocks.dart';

void main() {
  late UpdateDream usecase;
  late MockDreamRepository mockRepository;

  setUp(() {
    mockRepository = MockDreamRepository();
    usecase = UpdateDream(mockRepository);
  });
  ...
  test(
    ...
    () async {
      // arrange
      when(mockRepository.updateDream(tDream))
          .thenAnswer((_) async => const Right(unit));

      // act
      final result = await usecase(UpdateDreamParams(dream: tDream));

      // assert
      expect(result, const Right(unit));
      verify(mockRepository.updateDream(tDream));
